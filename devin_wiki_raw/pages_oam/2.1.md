...
The `Asset` interface is defined in [asset.go:7-11]() and consists of three methods that enable polymorphic handling of all asset types throughout the system:
...
The `AssetType` is defined as a string type in [asset.go:13]() to enable human-readable serialization and extensibility. Twenty-one asset type constants are defined in [asset.go:15-37](), representing the complete taxonomy of assets the model can describe.
...
**Sources:** [asset.go:15-37]()

### AssetList Array

A convenience array `AssetList` is provided in [asset.go:39-43]() containing all asset type constants. This array enables iteration over all defined asset types, useful for validation, documentation generation, or exhaustive testing.
...
**Sources:** [asset.go:8](), [file/file.go:21-23](), [file/file_test.go:14-21]()
...
**Sources:** [asset.go:9](), [file/file.go:26-28](), [file/file_test.go:23-34]()
...
The `File` struct defines JSON tags in [file/file.go:14-18]():
```go
type File struct {
    URL  string `json:"url"`
...
The `JSON()` method implementation in [file/file.go:31-33]():
```go
...
**Sources:** [asset.go:10](), [file/file.go:14-18](), [file/file.go:31-33](), [file/file_test.go:36-52]()
...
3. **Implement AssetType() method**
   - Return the corresponding constant from [asset.go:15-37]()
...
**Sources:** [file/file.go:14-33]()

---

## Interface Compliance Testing

The codebase uses Go's compile-time interface compliance checks to ensure all asset types properly implement the `Asset` interface. This pattern is demonstrated in [file/file_test.go:24-25]():
...
**Key() Method Test** ([file/file_test.go:14-21]()):
```go
...
**JSON() Method Test** ([file/file_test.go:36-52]()):
```go
...
**Sources:** [file/file_test.go:14-52](), [asset_test.go:19-25]()

---

## AssetType Validation

The [asset_test.go:27-85]() file contains comprehensive validation of all 21 asset type constants, ensuring:
...
The choice of `string` as the underlying type for `AssetType` ([asset.go:13]()) rather than an integer enumeration enables:
